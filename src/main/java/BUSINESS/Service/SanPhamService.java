...
public class SanPhamService implements ISanPham {
 ...
 public static SanPham AddSPDE(SanPham sanPhamThem) {
 
        InputStream.AppendDataFileSP(sanPhamThem);
        System.out.println(sanPhamThem.toString());
        System.out.println("Them Thanh Cong");
        return sanPhamThem;
    }
    @Override
  public SanPhamDto GetSanPhamDto(String masp) {
	  ...
	  for(int i = 0 ; i < list.size(); i++) {
		  if(list.get(i).getMasp().equals(masp)) {
			  ...
			  spt = new SanPhamDto(sptResponse.getMasp(),sptResponse.getTensp(),sptResponse.getGiaban(),sptResponse.getSoluongtonkho(),sptResponse.getMotasanpham(),sptResponse.getLoaisanpham());
  ...
  public SanPham DeleteSP(String masp) {
	  ...
	  if(sp == null) {
          ...
          SanPham so = new SanPham();
          InputStream.InputSanPham(so);
          for(SanPham i:this.list){
           if(!i.getMasp().equals(masp))
           //SanPhamService.AddSPDE(i);
  ...
  public SanPham EditSP(String masp) {
	...
	System.out.println("nhap ten loai san pham");
      ...
      sp.setManhacungcap(ncc);
      sp.setMotasanpham(mota);
      sp.setNhacungcap(tenncc);
